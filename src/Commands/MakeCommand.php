...
{
  const ARG_CLASS_NAME = 'class_name';
  const CLASS_TEMPLATE_FILE_PATH = __DIR__.'/../stubs/Command.stub';
  ...
  {
    $this->addArgument(self::ARG_CLASS_NAME, InputArgument::REQUIRED, 'Class name, E.g.: MyAwesomeCommand');
  }
  ...
  public function handle(InputInterface $input)
  {
    try {
      ...
      $content = str_replace('{{ $command }}', strtolower($className), $content);
